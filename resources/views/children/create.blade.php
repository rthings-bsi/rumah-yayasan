<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">{{ __('Add New Child') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Fill in the details below to register a new child.') }}</p>
        </div>
        ...
        <div class="content-card">
            ...
            <form action="{{ route('children.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="content-card-body border-b border-slate-100 dark:border-slate-700">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="registration_number" value="{{ old('registration_number') }}" class="form-input-modern" required placeholder="e.g. RH-2026-001">
                        </div>
                        <div>
                            <label class="form-label-modern">{{ __('Full Name') }}</label>
                            <input type="text" name="full_name" value="{{ old('full_name') }}" class="form-input-modern" required>
                        </div>
                        <div>
                            <label class="form-label-modern">{{ __('Place of Birth') }}</label>
                            <input type="text" name="place_of_birth" value="{{ old('place_of_birth') }}" class="form-input-modern" required>
                        </div>
                        <div>
                            <label class="form-label-modern">{{ __('Date of Birth') }}</label>
                            <input type="date" name="date_of_birth" value="{{ old('date_of_birth') }}" class="form-input-modern" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="enrollment_status" class="form-input-modern" required>
                                <option value="active" {{ old('enrollment_status') == 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                        ...
                        <div>
                            ...
                            <input type="date" name="admission_date" value="{{ old('admission_date') }}" class="form-input-modern" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="content-card-header">
                    ...
                    <h3 class="section-title">{{ __('Documents') }} <span class="text-sm font-normal text-slate-400">({{ __('Optional') }})</span></h3>
                </div>
                <div class="content-card-body border-b border-slate-100 dark:border-slate-700">
                    <div id="documents-container" class="space-y-3">
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-xl bg-slate-50/80 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                            <select name="documents[0][type]" class="form-input-modern sm:w-1/3">
                            ...
                            <input type="file" name="documents[0][file]" class="block flex-1 text-sm text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-600 hover:file:bg-indigo-100 file:cursor-pointer file:transition-colors">
                        </div>
                    </div>
