                            ...
                            @foreach($child->documents as $doc)
                                <div class="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-[10px] font-black uppercase text-slate-400 leading-none mb-1">{{ str_replace('_', ' ', $doc->type) }}</p>
                                            <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="text-xs font-bold text-slate-700 dark:text-slate-200 hover:text-indigo-600 transition-colors uppercase tracking-widest">{{ __('View File') }}</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
