<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto pb-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="glass-card overflow-hidden !rounded-[2.5rem] !bg-white/80 dark:!bg-slate-900/80 border-white/40 dark:border-slate-800/50 shadow-premium animate-fade-in-up delay-300">
            {{-- Search & Filter --}}
            <div class="p-5 md:p-8 border-b border-slate-100 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/20">
                <form method="GET" action="{{ route('children.index') }}" class="flex flex-col lg:flex-row gap-4 md:gap-5">
                    <div class="relative flex-1 group">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search name or registration number...') }}" 
                               class="form-input-modern pl-11 !border-slate-200 dark:!border-slate-700 focus:!ring-indigo-500/20 text-xs md:text-sm">
                    </div>
                    <div class="lg:w-80">
                        <select name="asrama_id" class="form-input-modern !border-slate-200 dark:!border-slate-700 focus:!ring-indigo-500/20 text-xs md:text-sm">
                    ...
                    <div class="flex gap-2 min-w-full md:min-w-0">
                        <button type="submit" class="btn btn-primary !rounded-2xl px-6 md:px-10 flex-1 md:flex-none justify-center !bg-slate-800 dark:!bg-indigo-600 hover:!bg-slate-900 dark:hover:!bg-indigo-700 transition-all shadow-lg group">
                            ...
                            <span class="font-bold text-xs uppercase tracking-widest">{{ __('Filter') }}</span>
                        </button>
                        @if(request('search') || request('asrama_id'))
                            <a href="{{ route('children.index') }}" class="btn btn-secondary !rounded-2xl border-slate-200 dark:border-slate-700 text-slate-500 px-6 flex-1 md:flex-none justify-center">
                                <span class="text-xs font-bold uppercase tracking-widest">{{ __('Reset') }}</span>
                            </a>
                        @endif
                    </div>
                </form>
            ...
            <div class="relative">
                ...
                <div class="hidden lg:block overflow-x-auto">
                    <table class="modern-table !border-none">
                        <thead>
                            <tr class="!bg-transparent text-slate-400 dark:text-slate-500">
                                ...
                                <tr class="row-interactive group/row transition-all duration-300">
                                    ...
                                    <td>
                                        <div class="flex flex-col">
                                            <span class="font-black text-slate-800 dark:text-white text-base tracking-tight group-hover/row:text-indigo-600 transition-colors">{{ $child->full_name }}</span>
                ...
                <div class="lg:hidden p-5 flex flex-col gap-4">
                    @forelse($children as $child)
                        @php
                            $profilePhoto = $child->documents->where('document_type', 'profile_photo')->first();
                        @endphp
                        <div class="bg-white dark:bg-slate-900/40 rounded-[2rem] p-4 border border-slate-100 dark:border-slate-800 shadow-sm relative group animate-fade-in-up">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center overflow-hidden border border-white/20 shadow-lg shrink-0">
                                    @if($profilePhoto)
                                        <img src="{{ Storage::disk('s3')->url($profilePhoto->file_path) }}" class="w-full h-full object-cover" alt="{{ $child->full_name }}">
                                    @else
                                        <span class="text-white font-black text-xs">{{ substr($child->full_name, 0, 1) }}</span>
                                    @endif
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-black text-slate-800 dark:text-white text-base truncate tracking-tight">{{ $child->full_name }}</h4>
                                    <div class="flex items-center gap-2 mt-0.5">
                                        <span class="font-mono text-[9px] font-black text-indigo-500 uppercase tracking-widest">{{ $child->registration_number }}</span>
                                        <span class="text-slate-300 dark:text-slate-700 text-[10px]">•</span>
                                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ __($child->gender) }}</span>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end shrink-0">
                                    <span class="w-2.5 h-2.5 rounded-full ring-4 ring-slate-50 dark:ring-slate-900/50
                                        {{ $child->enrollment_status == 'active' ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.4)]' : '' }}
                                        {{ $child->enrollment_status == 'graduated' ? 'bg-blue-500 shadow-[0_0_12px_rgba(59,130,246,0.4)]' : '' }}
                                        {{ $child->enrollment_status == 'withdrawn' ? 'bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.4)]' : '' }}"></span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 pt-3 border-t border-slate-50 dark:border-slate-800/50 flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    @if($child->asrama)
                                        <div class="flex items-center gap-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-900/50 px-2.5 py-1.5 rounded-xl border border-slate-100 dark:border-slate-800">
                                            <svg class="w-3 h-3 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                                            {{ $child->asrama->kode_asrama }}
                                        </div>
                                    @endif
                                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ __($child->category) }}</span>
                                </div>

                                <div class="flex items-center gap-1.5">
                                    <a href="{{ route('children.show', $child) }}" class="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-indigo-600 bg-slate-50 dark:bg-slate-900 rounded-xl transition-all border border-slate-100 dark:border-slate-800" title="View Profile">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    @if(auth()->user()->role === 'admin')
                                        <a href="{{ route('children.edit', $child) }}" class="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-amber-600 bg-slate-50 dark:bg-slate-900 rounded-xl transition-all border border-slate-100 dark:border-slate-800" title="Edit Record">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form action="{{ route('children.destroy', $child) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure you want to delete this record?') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-rose-600 bg-slate-50 dark:bg-slate-900 rounded-xl transition-all border border-slate-100 dark:border-slate-800" title="Delete">
                    ...
                    @empty
                        <div class="py-12 text-center">
                            <p class="text-slate-500 italic">{{ __('No children found matching your filters.') }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
