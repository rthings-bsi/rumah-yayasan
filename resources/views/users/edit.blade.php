<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            <span class="font-semibold text-slate-800 dark:text-slate-100">{{ __('Edit User Profile') }}</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto pb-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 animate-fade-in text-center">
            <h1 class="text-4xl font-black text-slate-800 dark:text-white tracking-tight">{{ __('Update Account Control') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">{{ __('Modify identity and security parameters for the selected user.') }}</p>
        </div>
                ...
                <div class="p-8 space-y-8">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3 pb-4 border-b border-slate-100 dark:border-slate-800/50">
                            ...
                            <h3 class="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">{{ __('Identity Profile') }}</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <x-text-input id="name" name="name" type="text" class="form-input-modern" :value="old('name', $user->name)" required autofocus />
                            ...
                            <div class="space-y-2">
                                ...
                                <x-text-input id="email" name="email" type="email" class="form-input-modern" :value="old('email', $user->email)" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <select id="role" name="role" class="form-input-modern h-[50px] cursor-pointer" required>
                                    <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>{{ __('Staff Member (Standard Access)') }}</option>
                                    <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>{{ __('Administrator (Full Access)') }}</option>
                                </select>
                                <x-input-error :messages="$errors->get('role')" class="mt-2" />
                            </div>

                            <div class="space-y-2">
                                <x-input-label for="password" :value="__('New Password (Optional)')" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1" />
                                <x-text-input id="password" name="password" type="password" class="form-input-modern" placeholder="{{ __('Leave blank to keep current') }}" />
                                <x-input-error :messages="$errors->get('password')" class="mt-2" />
                            </div>

                            <div class="md:col-start-2 space-y-2">
                                <x-input-label for="password_confirmation" :value="__('Confirm New Password')" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1" />
                                <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="form-input-modern" />
